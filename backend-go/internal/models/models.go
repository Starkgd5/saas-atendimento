...
// Método auxiliar para obter o estoque (calculado a partir dos lotes)
// Nota: O estoque total é calculado em tempo real via serviço
func (p *Produto) GetEstoqueTotal(lotes []Lote) int {
	total := 0
	for _, l := range lotes {
		if l.Status == "Ativo" {
			total += l.Quantidade
